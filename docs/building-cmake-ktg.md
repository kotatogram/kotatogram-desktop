...
The process is almost the same as with 14.04, but with one change: you need to use OpenSSL 1.0.2 instead of 1.0.1.
...
P.S. It may be bad idea to make app in multiple threads. In case of errors try omit `$MAKE_THREADS_CNT` parameter.

To make Release version go to ***BuildPath*/tdesktop/out/Release** and run

    make $MAKE_THREADS_CNT

Since binary file will be big, don't forget to run this in ***BuildPath*/tdesktop/out/Release** if you will publish the app:
